 ...
 * @note  save to eeprom.
 */
void side_light_contol(uint8_t brighten)
{
    if (brighten)
    {
 ...
 * @note  save to eeprom.
 */
void side_speed_contol(uint8_t fast)
{
...
void sys_led_show(void)
{
    ...
    if (host_keyboard_led_state().num_lock) {
        set_left_rgb(SIDE_BLINK_LIGHT, SIDE_BLINK_LIGHT, SIDE_BLINK_LIGHT);
    }

    if (host_keyboard_led_state().num_lock && host_keyboard_led_state().caps_lock) {
        r_temp = SIDE_BLINK_LIGHT;
        for (int i = 0; i < SIDE_LINE - 3; i++)
            rgb_matrix_set_color(SIDE_INDEX + i, r_temp, SIDE_BLINK_LIGHT, SIDE_BLINK_LIGHT);
        r_temp = 0X00;
        for (int i = 3; i < SIDE_LINE; i++)
            rgb_matrix_set_color(SIDE_INDEX + i, r_temp, SIDE_BLINK_LIGHT, SIDE_BLINK_LIGHT);
    }
}
